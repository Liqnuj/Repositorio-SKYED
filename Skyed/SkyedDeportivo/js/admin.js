...
const sectionColumns = {
  ...
  categorias: [
    { label: 'Nombre',      keys: ['nombre_cc'] },
    { label: 'Evento', keys: ['nombre_evento', 'evento_cc', 'id_e'] },
    { label: 'Edad mín.',   keys: ['edad_minima_cc'] },
    { label: 'Edad máx.',   keys: ['edad_maxima_cc'] },
    { label: 'Género',      keys: ['genero_cc'] },
    { label: 'Distancia',   keys: ['distancia_cc'] },
    { label: 'Descripción', keys: ['descripcion_cc'] },
    { label: 'Acciones', fn: row => `<div style="display:flex;gap:.4rem"><button class="btn btn-outline btn-sm" onclick='abrirModalCategoria(${JSON.stringify(row)})'>✏️</button><button class="btn btn-danger btn-sm">🗑️</button></div>`
    },
  ],  
  patrocinadores: [
