(function () {
  ...
  let currentAccent = localStorage.getItem("lumara-accent-social") || "#8827f0";
  ...
  function applyAccToggleColor() {
    const toggle = document.getElementById("accToggle");
    if (!toggle) return;
    const hasCustomAccent = localStorage.getItem("lumara-accent-social");
    if (hasCustomAccent) {
      toggle.style.background = currentAccent;
    } else {
      toggle.style.background = ""; // vuelve al degradado del CSS
    }
  }
  ...
  document.addEventListener("DOMContentLoaded", function () {
    ...
    document.getElementById("accReset").addEventListener("click", () => {
      ...
      // currentAccent = "#431888";
      currentAccent = "#8827f0";
      darkMode = false;
      ...
      document.querySelectorAll(".color-swatch").forEach((s) => s.classList.remove("active"));
      document.querySelector('[data-color="#8827f0"]')?.classList.add("active");
      document.getElementById("modeLight").classList.add("active");
      document.getElementById("modeDark").classList.remove("active");
