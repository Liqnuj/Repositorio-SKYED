...
$stmt = $pdo->prepare("SELECT id_u, nombre_u, apellido_u, correo_u, contrasena_u, estado_u, telefono_u, fecha_nacimiento_u, documento_u FROM usuario WHERE correo_u = ? LIMIT 1");
$stmt->execute([$email]);
...
if ($u['estado_u'] !== 'activo') {
    echo json_encode(['ok' => false, 'error' => 'Tu cuenta no está activa. Contacta soporte.']);
    exit;
}

$_SESSION['usuario_id'] = $u['id_u'];
